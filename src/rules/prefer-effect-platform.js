export default {
  meta: {
    ...
    messages: {
      preferPlatformHttp:
        'Use @effect/platform HttpClient instead of native fetch() or http/https modules. Import from "effect".',
      ...
      preferPlatformPath: 'Use @effect/platform Path instead of node:path. Import Path from "@effect/platform".',
  ...
  create(context) {
    return {
      ...
      CallExpression(node) {
        ...
        if (callee.type === "MemberExpression" && callee.object.type === "Identifier" && callee.object.name === "Bun") {
