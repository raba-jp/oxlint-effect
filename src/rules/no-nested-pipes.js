...
const noNestedPipes = {
  createOnce(context) {
    const checkForNestedPipes = (node) => {
      ...
      const findNestedPipe = (n, stopAtFunctionBoundary = false) => {
        ...
        if (n.type === "CallExpression" && n.callee.type === "Identifier" && n.callee.name === "pipe") {
          return n;
        }
        ...
        for (const key in n) {
          ...
          if (n[key] && typeof n[key] === "object") {
            if (globalThis.Array.isArray(n[key])) {
  ...
  meta: {
    type: "suggestion",
    docs: {
      description: "Forbid nested pipe() calls where one pipe is used as an argument to another pipe.",
    },
    messages: {
      noNestedPipes:
